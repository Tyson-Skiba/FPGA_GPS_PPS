...
rdata_qpsk=read_complex_binary1('in',10*(dl+1)); %data blocks separated by a single "1"
n=5; 
data=rdata_qpsk(n+(n-1)*dl+15+1:n+n*dl-1); %selecting the nth block

fftdata=fftshift(fft(data));
figure(2)
plot(20*log10(abs(fftdata)/(dl-16)));
grid on
figure(3)
plot(abs(rdata_qpsk));
% 
